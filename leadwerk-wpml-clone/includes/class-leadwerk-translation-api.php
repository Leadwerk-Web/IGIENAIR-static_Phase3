...
class Leadwerk_Translation_API {
	...
	public static function get_record_by_public_slug( $language_code, $public_slug, $type_like = 'post_%' ) {
		...
		// Bei gleichem public_slug (z. B. Seite "thats-igienair" + falsch registriertes Bild) muss die Seite vor Attachment/Revision gewinnen.
		$row = $wpdb->get_row(
			$wpdb->prepare(
	...
	public static function link_posts( $source_post_id, $translated_post_id, $target_lang, $translated_is_root = false, $args = array() ) {
		...
		$source_record = self::ensure_post_record(
			...
			array(
				...
				'is_home'       => ! empty( $args['source_is_home'] ) || 'igienair-home' === get_post_meta( $source_post->ID, 'leadwerk_source_key', true ) || (int) get_option( 'page_on_front' ) === (int) $source_post->ID,
				'status'        => 'complete',
			)
		);
