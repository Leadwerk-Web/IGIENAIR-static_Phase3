...
class Leadwerk_WPML_Clone {
	...
	public static function init() {
		...
		add_action( 'init', array( __CLASS__, 'maybe_repair_igienair_body_classes' ), 5 );
	...
	public static function handle_admin_actions() {
		...
		if ( isset( $_GET['leadwerk_repair_registry'] ) && current_user_can( 'manage_options' ) && wp_verify_nonce( wp_unslash( $_GET['_wpnonce'] ?? '' ), 'leadwerk_repair_registry' ) ) {
			$summary = array(
				...
				'styles'   => self::repair_igienair_body_class_meta( true ),
			);
	...
	protected static function create_translation_post( $source_id, $lang ) {
		...
		if ( $source->post_parent ) {
			$translated_parent = (int) Leadwerk_Translation_API::get_translation( (int) $source->post_parent, $lang );
		}
	 ...
	 * @return void
	 */
	public static function maybe_repair_igienair_body_classes() {
		if ( self::STYLE_REPAIR_VERSION === (string) get_option( self::STYLE_REPAIR_OPTION, '' ) ) {
			return;
		}

		$summary = self::repair_igienair_body_class_meta();
	...
	/**
	 * Backfill canonical body classes for all importer-managed IGIENAIR pages.
	 *
	 * @param bool $force Whether to bypass the stored repair version.
	 * @return array<string,int>
	 */
	protected static function repair_igienair_body_class_meta( $force = false ) {
		$summary = array(
			'scanned'      => 0,
			'repaired'     => 0,
			'matched'      => 0,
