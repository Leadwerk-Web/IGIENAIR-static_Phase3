...
class SubHeadingsKeywordAssessment extends _assessment.default {
  ...
  hasGoodNumberOfMatches() {
    return (0, _inRange.inRangeStartEndInclusive)(this._subHeadingsResearchResult.matches, this._minNumberOfSubheadings, this._maxNumberOfSubheadings);
  }
  ...
  getResultForNoSubheadings() {
    ...
    if (textLength >= this._config.parameters.recommendedMaximumLength) {
      return {
        ...
        (0, _i18n.__)("%1$sKeyphrase in subheading%3$s: You are not using any higher-level subheadings containing the keyphrase or its synonyms. %2$sFix that%3$s!", "wordpress-seo"), this._config.urlTitle, this._config.urlCallToAction, "</a>")
      };
    }
    if (textLength < this._config.parameters.recommendedMaximumLength) {
      return {
        score: this._config.scores.goodShortTextNoSubheadings,
        resultText: (0, _i18n.sprintf)(/* translators: %1$s expands to a link on yoast.com and %2$s expands to the anchor end tag. */
        (0, _i18n.__)("%1$sKeyphrase in subheading%2$s: You are not using any higher-level subheadings containing the keyphrase or its synonyms, but your text is short enough and probably doesn't need them.", "wordpress-seo"), this._config.urlTitle, "</a>")
      };
    }
  }
  ...
  calculateResult(paper) {
    if (!paper.hasKeyword() || !paper.hasText()) {
      return {
        ...
        resultText: (0, _i18n.sprintf)(/* translators: %1$s and %2$s expand to a link on yoast.com, %3$s expands to the anchor end tag. */
        (0, _i18n.__)("%1$sKeyphrase in subheading%3$s: %2$sPlease add both a keyphrase and some text to receive relevant feedback%3$s.", "wordpress-seo"), this._config.urlTitle, this._config.urlCallToAction, "</a>")
      };
    }
    ...
    if (this.isOneOfOne()) {
      return {
        ...
        (0, _i18n.__)("%1$sKeyphrase in subheading%2$s: Your H2 or H3 subheading reflects the topic of your copy. Good job!", "wordpress-seo"), this._config.urlTitle, "</a>", this._subHeadingsResearchResult.matches)
      };
    }
    if (this.hasGoodNumberOfMatches()) {
      return {
        ...
        /* translators: %1$s expands to a link on yoast.com, %2$s expands to the anchor end tag,
        %3$d expands to the number of subheadings containing the keyphrase. */
        (0, _i18n._n)("%1$sKeyphrase in subheading%2$s: %3$s of your H2 and H3 subheadings reflects the topic of your copy. Good job!", "%1$sKeyphrase in subheading%2$s: %3$s of your H2 and H3 subheadings reflect the topic of your copy. Good job!", this._subHeadingsResearchResult.matches, "wordpress-seo"), this._config.urlTitle, "</a>", this._subHeadingsResearchResult.matches)
      };
    }
    return {
